...
void Camera::Pitch(float angle)
{
	...
	DirectX::XMStoreFloat3(&mUp,   XMVector3TransformNormal(DirectX::XMLoadFloat3(&mUp), R));
	DirectX::XMStoreFloat3(&mLook, XMVector3TransformNormal(DirectX::XMLoadFloat3(&mLook), R));
}
...
void Camera::RotateY(float angle)
{
	...
	DirectX::XMStoreFloat3(&mRight,   XMVector3TransformNormal(DirectX::XMLoadFloat3(&mRight), R));
	DirectX::XMStoreFloat3(&mUp, XMVector3TransformNormal(DirectX::XMLoadFloat3(&mUp), R));
	DirectX::XMStoreFloat3(&mLook, XMVector3TransformNormal(DirectX::XMLoadFloat3(&mLook), R));
}
